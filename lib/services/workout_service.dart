...
class WorkoutService {
  ...
  Future<WorkoutSession> startWorkoutFromTemplate(WorkoutTemplate template) async {
    ...
    // ExerciseSession lista létrehozása a sablon alapján
    final exerciseSessions = template.exerciseTemplates.map((exerciseTemplate) {
      return ExerciseSession(
        exerciseTemplateId: exerciseTemplate.id,
        exerciseName: exerciseTemplate.name,
        sets: [], // Üres szettek kezdéskor
      );
    }).toList();
  ...
  Future<void> completeSession(WorkoutSession session, {String? notes}) async {
    ...
    // Sablon frissítése az új súlyokkal
  ...
  Future<void> _updateTemplateFromSession(WorkoutSession session) async {
    ...
    for (final exerciseTemplate in template.exerciseTemplates) {
      // Keressük meg a megfelelő exercise sessiont
      final exerciseSession = session.exerciseSessions.firstWhere(
        (es) => es.exerciseTemplateId == exerciseTemplate.id,
      ...
      if (exerciseSession.sets.isNotEmpty) {
        // Az utolsó (legjobb) szett alapján frissítjük
        final lastSet = exerciseSession.sets.last;
        final updatedExerciseTemplate = exerciseTemplate.updateWithSet(
          weight: lastSet.weight,
          reps: lastSet.reps,
          performedAt: session.completedAt ?? DateTime.now(),
        );
        updatedExerciseTemplates.add(updatedExerciseTemplate);
      } else {
        // Ha nem volt szett, megtartjuk az eredetit
        updatedExerciseTemplates.add(exerciseTemplate);
      }
    }
  ...
  Future<List<WorkoutSession>> getRecentSessions({int days = 30}) async {
    ...
    return sessions
        .where((s) => s.startedAt.isAfter(cutoffDate))
        .toList()
  ...
  Future<void> createDefaultTemplates() async {
    final existingTemplates = await loadTemplates();
    ...
    print('Alapértelmezett sablonok létrehozva: ${defaultTemplates.length} db');
  }
}